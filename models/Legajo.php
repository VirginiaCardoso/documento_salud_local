 ...
 */
class Legajo extends \yii\db\ActiveRecord
{
    /**
     * @inheritdoc
     */
    ...
    {
        return $this->hasMany(Medico::className(), ['ME_LEGAJO' => 'LE_NUMLEGA']);
    }

    public function getMedicoGuardia()
    {
        $matriculas = $this->matriculas;
        foreach ($matriculas as $matricula) {
            if ($matricula->ME_ESPEC == $matricula->ME_GUARDIA)
                return $matricula;
        }

        if ($matriculas != null)
            return array_values($matriculas)[0];
        else
            return null;
    }
    
    public static function getListaPersonal()
    {
        return ArrayHelper::map(Legajo::find()->asArray()->all(), 'LE_NUMLEGA', 
                                    function($model, $defaultValue) {
                                        return '[' . $model['LE_NUMLEGA'] . '] ' . $model['LE_APENOM'];
                                    });
    }

    function getHash(){
        $fecha = new \DateTime();
        $input = 'stringderelleno' . $this->LE_NUMLEGA . $fecha->format('Y-m-d');
        return hash('sha256', $input, false);
    }
    
    public function getGrupoIntranet()
    {
        return $this->hasOne(GrupoIntranet::className(), ['id_grupo' => 'grupo']);
    }
}
